...
# -1 is for excluding anonymous entries!
number_of_authors = git_log['author'].unique().size - 1
...
top_10_authors = git_log.groupby('author').count()
top_10_authors = top_10_authors.sort_values(by='timestamp', ascending=False)
top_10_authors = top_10_authors[0:10]
...
last_commit_timestamp = '2018-10-26 19:32:00'
...
#from: https://stackoverflow.com/questions/25146121/extracting-just-month-and-year-from-pandas-datetime-column-python
#if you don't need the exact timestamp, and rather, just the year
#commits_per_year = corrected_log.groupby(corrected_log['timestamp'].dt.year).count()

#including the timestamp of the first commit per year 
...
# plot the data
#we omit # of commits from the last year, 2017, because it is incomplete 
ax = commits_per_year.plot(title='# of Commits Per Year',legend=False, xlim=(pd.Timestamp('2005-01-01'),pd.Timestamp('2016-01-01')))
ax.set(xlabel='Year', ylabel='commits')
...
year_with_most_commits = commits_per_year.sort_values(by='commits', ascending=False).index[0].year
